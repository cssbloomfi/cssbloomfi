...
class Zend_Scheduler
{
    ...
    static public $availableBackends = array('File');
    ...
    /**
     * Serializes all tasks for convenient storage in a file or 
     * database.
     *
     * @return string All tasks as a serialized string
     */
    public function serializeTasks()
    {
        return serialize($this->_tasks);
    }
    ...
    public function run(Zend_Controller_Front $controller = null)
    {
        ...
        foreach ($tasks as $name => $task) {
            if ($this->_limit == 0 or $completed < $this->_limit) {
                $responses[$name] = $task->run($this->_controller);
                array_shift($tasks);
                $completed++;
            } else {
                break;
