...
if(!class_exists('Bloomfi_HTML2FPDF_FPDF'))
{
define('FPDF_VERSION','1.52');

class Bloomfi_HTML2FPDF_FPDF
{
//Private properties
...
*                                                                              *
*******************************************************************************/
function Bloomfi_HTML2FPDF_FPDF($orientation='P',$unit='mm',$format='A4')
{
	//Some checks
	$this->_dochecks();
...
{
	//Fatal error
	die('<B>Bloomfi_HTML2FPDF_FPDF error: </B>'.$msg);
}
...
function _putinfo()
{
	$this->_out('/Producer '.$this->_textstring('Bloomfi_HTML2FPDF_FPDF '.FPDF_VERSION));
	if(!empty($this->title))
