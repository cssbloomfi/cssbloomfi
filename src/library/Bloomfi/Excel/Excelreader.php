...
* Main Class
*/
class Bloomfi_Excel_Excelreader {

	public $error; //(Smritiman)
	public $readable=1; //(Smritiman)
	 ...
	 */
	function Bloomfi_Excel_Excelreader($file='',$store_extended_info=true,$outputEncoding='') {
		/*--------------------------------------------------------------------------------
		   PERPOSE :  Permanent    |    ACTION:    |    DATE: 4/14/2010   |   By: Smritiman
		-----------------------------------------------------------------------------------
		...
		--------------------------------------------------------------------------------*/
		$this->_ole = new OLERead();
		$this->tools= new Bloomfi_Excel_XtrTools; //(Smritiman)
		...
		if ($file!="") {
			$this->read($file);
			if($this->readable==0) {
				$this->error= 'The filename "<b>' . $file . '</b>" is not readable'; //(Smritiman)
			}
		}
	}
