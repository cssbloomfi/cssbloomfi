<?php
	...
	Class Bloomfi_Excel_Excelwriter
	{
		...
		*/
		function open($file)
		{
			...
			{
				$this->error="Usage : New Bloomfi_App_Excelwriter('fileName')";
				echo $this->error;
				return false;
			}
			if($this->fp==false)
...
EOH;
		...
		*/
		...
		function writeLine($line_arr)
		{
			...
			{
				/*  This section modified by Smritiman Barua*/
				if( is_numeric($col))
					$ex = 'x:num="'.$col.'"';
				else
					$ex = " ";

				fwrite($this->fp,"<td class=xl24 width=64 $ex>".$col."</td>");
			}
			fwrite($this->fp,"</tr>");
		}
		...
		*/
		function writeCol($value)
		{
			...
			fwrite($this->fp,"<td class=xl24 width=64 $ex>".$value."</td>");
		}
	}
?>