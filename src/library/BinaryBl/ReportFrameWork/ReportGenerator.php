...
class BinaryBl_ReportFrameWork_ReportGenerator extends  BinaryBl_ReportFrameWork_XmlContainer
{
	...
	protected $_groupByCondition=null;
	protected $_addtionalFilter=null;
	...
	public function __construct($data)
	{
		...
		$this->_enterprise=$this->_session->enterpriseId;
		//$this->_session->SESSION_ENTERPRISE_ID='ENTP_1';
		$this->contructDefaultSql();
		$this->createForm();
	}

	//
	public function contructDefaultSql()
	{
		//print_r($this->_xpathRoots['result']);
		//print_r($this->_xmlResultData);
		$groupBy=$addtionalParams=$groupByOptional=$orderBy=null;
		$this->functionCallLog('contructDefaultSql');
		$mainSql=$this->_xpathResultValue['sql'];
		if (isset($this->_xpathResultValue['defaultfilters']) && !empty($this->_xpathResultValue['defaultfilters'])){
			foreach($this->_xpathResultValue['defaultfilters'] as $filter){
					$this->loadXmlDefaultSqlValue($filter);
					$addtionalParams.=$this->getDefaultSqlParams(); }
		}
		if (isset($this->_xpathResultValue['sqlgroupby']) && !empty($this->_xpathResultValue['sqlgroupby'])){
			$groupByOptional=' GROUP BY '.$this->_xpathResultValue['sqlgroupby'];
		}
		$groupBy=$this->sqlGroupByCondition('PARENT');
		$orderBy=$this->sqlOrderByCondition('PARENT');
		$sql=$mainSql.$addtionalParams.$groupBy.$groupByOptional.$orderBy;
		$this->_xpathResultValue['sql']=$sql;
	}

	public function getDefaultSqlParams()
	{
		$tableName=$addParams=null;
		$valuesource=$this->_xpathValue['valuesource'];
		if(strtoupper($valuesource)=='SESSION'){
			$sessionVariable=$this->_xpathValue['sessionvariable'];
			$value=$this->_session->$sessionVariable;
			$column=$this->_xpathValue['columnname'];
			$operator=$this->_xpathValue['filtertype'];
			$valueSourceType=$this->_xpathValue['sessionvariabletype'];
			if ( isset($this->_xpathValue['tablename'])&&!empty($this->_xpathValue['tablename']) ) {
				$tableName=$this->_xpathValue['tablename'].".";
			}
			if ( isset($this->_xpathValue['applymode'])&&!empty($this->_xpathValue['applymode']) ) {
				if (strtoupper($this->_xpathValue['applymode'])=='EXTERNAL'){
					//die("EXTERNAL");
					$tableName="parent."; }
			}
			if(strtoupper($valueSourceType)=='CHAR') {
				$addParams = ' AND '.$tableName.$column.' '.$operator.' "'.$value.'"';
			}else
				$addParams = ' AND '.$tableName.$column.' '.$operator.' '.$value;
			...
			if ( isset($this->_xpathValue['applymode'])&&!empty($this->_xpathValue['applymode']) ) {
				if (strtoupper($this->_xpathValue['applymode'])=='EXTERNAL'){
					//die("EXTERNAL");
					$this->_decodeSQLConditionCodes($addParams);
					$this->_addtionalFilter.=$addParams;
				}
			}else return $addParams;
		}
	}
	...
	protected function _createSqlSearchBundle()
	{
		...
		if(isset($this->_xpathValue['dbtblcolumn']) && isset($this->_xpathValue['conditiontype'])) {{
				...
				$str.=$searchVariable;
				if(isset($this->_xpathValue['extend'])){
					$str.=' '.$this->_xpathValue['extend'];
				}
	...
	public function setBasics($type='TEXT')
	{
		...
		if($type=='TEXT' || $type=='SUBMIT' || $type=='DROPDOWN'){
			...
			else if($type=='TEXT') $length=' SIZE=10 ';
			if($type!='DROPDOWN')
	...
	//Comment : TODO
	public function sqlGroupByCondition($link=null)
	{
		$this->functionCallLog('sqlGrderByCondition');
		$this->clearPattern();
		$this->setPattern();
		$this->preAddToStringDirect(" GROUP BY ");

		if(strtoupper($link)!='PARENT')
		{
			if(!isset($this->_xpathResultValue['groupby']) &&
				empty($this->_xpathResultValue['groupby']))
				return;
			else {
				foreach($this->_xpathResultValue['groupby'] as $value)
							 $this->insertDataToPattern("parent.".$value);
				$this->getPatternString();
				$groupby=$this->getPatternString();
				if(isset($this->_xpathResultValue['groupby']->have) &&
					!empty($this->_xpathResultValue['groupby']->have) )
					$groupby.=' HAVING('.$this->_xpathResultValue['groupby']->have.') ';
				$this->_groupByCondition==$groupby;
			}
		}
		else
		{
			if(isset($this->_xpathResultValue['groupbyparent']) && !empty($this->_xpathResultValue['groupbyparent']))
			{
				foreach($this->_xpathResultValue['groupbyparent']->item as $value)
						 $this->insertDataToPattern($value);
				$groupby=$this->getPatternString();
				if(isset($this->_xpathResultValue['groupbyparent']->have) &&
					!empty($this->_xpathResultValue['groupbyparent']->have) )
					$groupby.=' HAVING('.$this->_xpathResultValue['groupbyparent']->have.') ';
				return $groupby;
			}
			else return;
		}
	...
	//Comment : TODO
	public function sqlOrderByCondition($link=null)
	{
		...
		{
			if(isset($this->_xpathResultValue['orderbyparent']) && !empty($this->_xpathResultValue['orderbyparent']))
			{
				foreach($this->_xpathResultValue['orderbyparent'] as $order) {
					foreach ( $order->item as $value)
						$this->insertDataToPattern($value);
					$this->postAddToStringDirect(" ".$order->type); }
	...
	public function constructSqlFromXml($search=true,$order=true,$limit=true)
	{
		...
		$sql=$sql.$str." FROM ( ".$this->_xpathResultValue['sql']." ) parent WHERE 1=1 ".$this->_addtionalFilter;
		...
		if(isset($this->_xpathResultValue['directCondition'])){
			$sql.=' '.$this->_xpathResultValue['directCondition'];
		}
		if($search==true){
		...
		if($order==true){
			...
			$sql=$sql.$this->_groupByCondition.$this->_orderByCondition; }
	...
	function _getFooterSql($params=null)
	{
		if(isset($this->_xpathResultValue['footersql']) && !empty($this->_xpathResultValue['footersql']))
		{
			$sqlContant= $this->_xpathResultValue['footersql'];
			$sql= $sqlContant->sqlfootercontent;
			...
			if($params){
				...
				foreach($params as $param){
					if($param==null)$param='%';
						...
						$check=$this->_checkAsterix($param);
						if($check==true){
							$param=str_replace('*','%',$param);
						}
					}
					$sql=str_replace(':'.$parameters[$i++],$param,$sql);
				}
			}
			
		//	$this->functionCallLog('getFooterSql -> '.$sql);
			//echo $sql;
		//	die;
			$this->_decodeSQLConditionCodes($sql);
			return $this->_db->fireSqlFetchAll($sql);
		}else return null;
	}
