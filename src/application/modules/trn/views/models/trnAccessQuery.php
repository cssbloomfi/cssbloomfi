...
class modules_trn_models_trnAccessQuery extends BinaryBl_XmlSqlBuilder
{
	...
	*/
    ...
    {
		return $this->db->fireFastSqlQuery("SAVEPOINT $savePoint");
	}
	...
	*/
    ...
    {
		return $this->db->fireFastSqlQuery("ROLLBACK TO SAVEPOINT $savePoint");
	}
	...
	*/
	...
	/*
		...
		------------------------------------------------------------------------
	*/
	public function waitAndAllow($string)
	{
		$phase1_microSec=rand(100,2000);
		usleep($phase1_microSec);
		...
		if($allow==1) {
			...
			for($i=0;$i<$phase1_limit;$i++){
				$allow=$this->isLock($string);
				if($allow==1){ $this->getLock($string);
					return $allow;}
